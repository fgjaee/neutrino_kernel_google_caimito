...
static DEVICE_ATTR_RW(hs_phy_tune);

static ssize_t
phy_tune_show(struct device *dev,
	      struct device_attribute *attr, char *buf)
{
	struct exynos_usbdrd_phy *phy_drd = dev_get_drvdata(dev);
	int len = 0, i, ret;
...
static ssize_t
phy_tune_store(struct device *dev,
	       struct device_attribute *attr, const char *buf, size_t n)
{
	char tune_name[30];
	u32 tune_val;
...
static int exynos_usbdrd_phyclk_get(struct exynos_usbdrd_phy *phy_drd)
{
	...
	if (!strcmp("none", phyclk_ids[0])) {
		...
		phy_drd->phy_clocks = devm_kmalloc(dev,
	...
	clk_ids = devm_kmalloc(dev, (clk_count + 1) * sizeof(const char *),
			       GFP_KERNEL);
	...
	phy_drd->clocks = devm_kmalloc(dev,
...
static void exynos_usbdrd_pipe3_init(struct exynos_usbdrd_phy *phy_drd)
{
	...
	if (gpio_is_valid(phy_drd->phy_port)) {
		if (phy_drd->reverse_phy_port)
			...
			value = gpio_get_value(phy_drd->phy_port);
		phy_drd->usbphy_info.used_phy_port =
				phy_drd->usbphy_sub_info.used_phy_port = value;
		dev_info(phy_drd->dev, "%s: phy port[%d]\n", __func__,
			 phy_drd->usbphy_info.used_phy_port);
	} else {
		...
		if (gpio_is_valid(phy_drd->phy_port)) {
			...
			if (phy_drd->reverse_phy_port)
				...
				value = gpio_get_value(phy_drd->phy_port);
			phy_drd->usbphy_info.used_phy_port = value;
			phy_drd->usbphy_sub_info.used_phy_port = value;
	...
	for (ret = 100; ret != 0; ret--) {
		...
		pll_lock = phy_exynos_usbdp_g2_check_pll_lock(&phy_drd
							->usbphy_sub_info);
		if (!pll_lock)
			break;
	}
	...
	for (ret = 100; ret != 0; ret--) {
		...
		pll_lock = phy_exynos_usbdp_g2_rx_cdr_pll_lock(&phy_drd
							->usbphy_sub_info);
		if (!pll_lock)
			break;
	}
...
#if defined(CONFIG_PHY_SAMSUNG_USB_GEN2_V4)
				   ...
				   int option, void *info)
{
	...
	case SET_DPPULLUP_ENABLE:
		phy_exynos_usb_v3p1_enable_dp_pullup(&phy_drd->usbphy_info);
		break;
	case SET_DPPULLUP_DISABLE:
		 phy_exynos_usb_v3p1_disable_dp_pullup(&phy_drd->usbphy_info);
		break;
...
static irqreturn_t exynos_usbdrd_phy_wakeup_interrupt(int irq, void *_phydrd)
{
	...
	dev_info(phy_drd->dev, "[%s] rewa sys valid set : %s\n",
...
static int exynos_usbdrd_phy_probe(struct platform_device *pdev)
{
	...
	if (ret)
		dev_err(dev,
			"%s - Couldn't create sysfs for HS PHY tune\n",
			__func__);
